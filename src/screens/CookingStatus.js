...
const CookingStatus = ({ route, navigation }) => {
  ...
  const [hasCompletedCooking, setHasCompletedCooking] = useState(false);
  ...
  useEffect(() => {
    ...
    const progressCallback = (data) => {
      ...
      if (data.progress !== undefined) {
        ...
        // Save cooking history when cooking reaches 100% (stage 5)
        if (data.progress >= 5 && !hasCompletedCooking && selectedQuantity) {
          setHasCompletedCooking(true);
          const cookingEntry = {
          ...
          storageService.saveCookingHistory(cookingEntry)
            .then(success => {
              if (success) {
                console.log('✅ Cooking history saved successfully');
                Alert.alert(
                  'Cooking Complete!',
                  `Your ${selectedQuantity} cups of rice is ready!`,
                  [{ text: 'OK' }]
                );
              } else {
                console.log('❌ Failed to save cooking history');
              }
            })
            .catch(error => {
  ...
  }, [isConnected, hasCompletedCooking, selectedQuantity]);
  ...
  const getStatusText = () => {
    switch (cookingStatus) {
      ...
      case 'drain': return 'Cooking Rice';
      case 'draining': return 'Cooking Rice';
      case 'steaming': return 'Cooking Rice';
